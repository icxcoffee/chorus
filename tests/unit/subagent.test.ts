...
describe("subagent", () => {
  it("parses message and usage events", () => {
    const parsed = parseSubagentNdjson(
      [
        ...
        JSON.stringify({ type: "usage", usage: { input: 1, output: 2, cost: { total: 0.03 } } })
      ].join("\n")
    );
    expect(parsed.output).toBe("hello");
    expect(parsed.usage).toEqual({ input: 1, output: 2, cacheRead: 0, cacheWrite: 0 });
      ...
      [
        ...
        JSON.stringify({
          ...
          message: {
            ...
            usage: { input: 3, output: 4, cacheRead: 1, cacheWrite: 2, cost: { total: 0.05 } }
          }
        })
      ].join("\n")
    );
    expect(parsed.output).toBe("/Users/icx/ai-workspace/chorus");
    expect(parsed.usage).toEqual({ input: 3, output: 4, cacheRead: 1, cacheWrite: 2 });
  ...
  it("falls back to Pi json text update events", () => {
    ...
    expect(parsed.output).toBe("hello");
    expect(parsed.usage).toEqual({ input: 5, output: 6, cacheRead: 0, cacheWrite: 0 });
  ...
  it("builds an activity log from assistant and tool events", () => {
    const parsed = parseSubagentNdjson(
      [
        ...
        JSON.stringify({
          type: "message_update",
          assistantMessageEvent: { type: "thinking_start", contentIndex: 0, partial: { content: [] } }
        }),
        JSON.stringify({
          type: "message_update",
          assistantMessageEvent: { type: "thinking_delta", contentIndex: 0, delta: "private reasoning", partial: { content: [] } }
        }),
        JSON.stringify({
          type: "message_update",
          assistantMessageEvent: { type: "text_delta", contentIndex: 1, delta: "checking files", partial: { content: [] } }
        }),
        JSON.stringify({
          type: "message_update",
          assistantMessageEvent: {
            type: "toolcall_end",
            contentIndex: 2,
            toolCall: { name: "grep", arguments: { pattern: "runChorus", path: "src" } },
            partial: { content: [] }
          }
        }),
        JSON.stringify({
    ...
    expect(parsed.activityLog).toContain("[thinking] receiving hidden reasoning");
  ...
  it("spawns pi with model and prompt files then succeeds without prompt in argv", async () => {
    ...
    child.stdout.write(`${JSON.stringify({ usage: { input: 1, output: 1, cost: { total: 0.01 } } })}\n`);
    child.emit("close", 0);
    ...
    expect(argv.some((arg) => arg.startsWith("@") && arg.endsWith("task.md"))).toBe(true);
  ...
  it("emits partial output from streaming Pi json events", async () => {
    ...
    await vi.waitFor(() => expect(spawn).toHaveBeenCalled());
    child.stdout.write(`${JSON.stringify({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: "hel" } })}\n`);
    child.stdout.write(`${JSON.stringify({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: "lo" } })}\n`);
    child.stdout.write(`${JSON.stringify({ type: "message_end", message: { role: "assistant", content: "hello" } })}\n`);
    child.emit("close", 0);
    const result = await promise;
    expect(result.status).toBe("success");
  ...
  it("emits activity logs while streaming Pi json events", async () => {
    ...
    await vi.waitFor(() => expect(spawn).toHaveBeenCalled());
    child.stdout.write(`${JSON.stringify({ type: "tool_execution_start", toolCallId: "t1", toolName: "read", args: { path: "src/index.ts" } })}\n`);
    child.stdout.write(`${JSON.stringify({ type: "message_update", assistantMessageEvent: { type: "text_delta", contentIndex: 0, delta: "read complete", partial: { content: [] } } })}\n`);
    child.stdout.write(`${JSON.stringify({ type: "message_end", message: { role: "assistant", content: "read complete" } })}\n`);
    child.emit("close", 0);
    const result = await promise;
    expect(result.status).toBe("success");
  ...
  it("turns empty successful pi output into an error", async () => {
    ...
    await vi.waitFor(() => expect(spawn).toHaveBeenCalled());
    child.stdout.write(`${JSON.stringify({ type: "message_end", message: { role: "assistant", content: [] } })}\n`);
    child.emit("close", 0);
    const result = await promise;
    expect(result.status).toBe("error");
