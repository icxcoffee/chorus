...
import type { ChorusProgress, ChorusResult, ChorusRunConfig, ModelInfo, PartialVoiceProgress, RegistryLike, VoiceResult } from "./types.js";
...
import { appendHistory as appendHistoryDefault, type StorePaths } from "./store.js";
...
export async function runChorus(args: RunChorusArgs): Promise<ChorusResult> {
  ...
  const conductorTimeoutMs = args.conductorTimeoutMs ?? DEFAULT_CONDUCTOR_TIMEOUT_MS;
  const voiceTasks = args.runConfig.voices.map((voice, voiceIndex) => async (): Promise<VoiceResult> => {
    const onProgress = (update: PartialVoiceProgress) => args.onProgress?.([update]);
  ...
  const settled = args.runConfig.mode === "subagent"
    ? await settleWithConcurrency(voiceTasks, DEFAULT_SUBAGENT_CONCURRENCY)
  ...
  if (successfulVoices < 2) {
    ...
    args.onProgress?.([{ kind: "conductor", conductor: args.runConfig.conductor, status: "running" }]);
    try {
      const synthesizeResult = await withTimeout((signal) => args.synthesisMode === "agent"
        ...
        : (args.synthesizeFn ?? synthesize)({
          ...
          signal,
            ...(args.optimizedPrompt ? { optimizedPrompt: args.optimizedPrompt } : {}),
            ...(args.fetchImpl ? { fetchImpl: args.fetchImpl } : {})
          }), conductorTimeoutMs, args.signal);
      synthesisText = synthesizeResult.synthesis;
      conductorUsage = synthesizeResult.usage;
      conductorCostUsd = synthesizeResult.costUsd;
      const activityLog = (synthesizeResult as { activityLog?: unknown }).activityLog;
      conductorActivityLog = typeof activityLog === "string" ? activityLog : undefined;
      args.onProgress?.([{
        kind: "conductor",
        conductor: args.runConfig.conductor,
        status: "success",
        ...
        ...(conductorActivityLog ? { activityLog: conductorActivityLog } : {})
      }]);
    } catch (error) {
  ...
  let result: ChorusResult = {
    ...resultBase,
    ...(conductorExecuted && conductorCostUsd !== undefined ? { conductorCostUsd } : {}),
  ...
  if (args.artifactDir) {
    result = await writeRunArtifacts({ result, outputDir: args.artifactDir, actorLabel: args.runConfig.mode === "subagent" ? "agent" : "voice" });
  }
  const appendHistory = args.appendHistory ?? ((entry: ChorusResult) => appendHistoryDefault(entry, args.storePaths));
  void appendHistory(result).catch((error) => {
    console.error("chorus history append failed:", error);
  });
  return result;
}
...
async function settleWithConcurrency<T>(
  ...
  return results.map((result, index) => {
    ...
    return { status: "rejected", reason: new Error(`voice task ${index} did not run`) };
  });
}
