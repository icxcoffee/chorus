...
export async function handleChorusCommand(
    ...
    notify(
        ctx,
        "Usage: /chorus config | /chorus ask <question> | /chorus agent <task> | /chorus jobs | /chorus watch <jobId> [agent] | /chorus cancel <jobId> | /chorus history prune [max-entries] | /chorus optimize <prompt>",
        "warning",
    );
}
